...
if analysis_mode == "📊 Single Venue":
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
        "📊 Venue Overview", "🏆 Team Performance", "🏏 Batting Trends",
        "📈 Match Statistics", "👑 Player Stats", "🔍 Insights"
    ])
...
- **Date Range:** {venue_matches['date'].min()} to {venue_matches['date'].max()}"""
        ...
        with col2:
            ...
            fig_wins = px.pie(winner_dist.reset_index(), values='count', names='winner',
                             title="Winners at This Venue", hole=0.3)
    ...
    with tab2:
        ...
        for team in all_teams:
            ...
            team_stats.append({'Team': team, 'Matches': total, 'Wins': wins, 'Losses': total - wins,
        ...
        with col1:
            ...
            fig_matches = px.bar(team_df.head(15), x='Team', y='Matches',
                                title="Top Teams by Matches", color='Matches',
        ...
        with col2:
            ...
            fig_wr = px.bar(best_teams, x='Team', y='Win %', title="Best Win % at Venue",
    ...
    with tab3:
        ...
        with col1:
            ...
            for season in venue_matches['season'].unique():
                ...
                sr_by_season.append({'Season': season, 'Strike Rate': sr})
            
            sr_df = pd.DataFrame(sr_by_season).sort_values('Season')
            fig_sr = px.line(sr_df, x='Season', y='Strike Rate',
                            title="Strike Rate Trend Over Seasons", markers=True)
        ...
        with col2:
            ...
            fig_bat = px.bar(top_batsmen, x='Player', y='Runs', title="Top 10 Scorers at Venue",
        ...
        with col2:
            ...
            boundary_text = f"""**Boundary Distribution**
- **Fours:** {fours}
- **Sixes:** {sixes}
- **Singles:** {singles}
- **4/6 Ratio:** {fours/sixes if sixes > 0 else 0:.2f}
...
- Avg Runs/Match: {(venue_deliveries['total_runs'].sum() / len(venue_matches)):.0f}"""
        ...
        with col2:
            ...
            batting_text = f"""**Batting Metrics**
...
- Strike Rate: {(venue_deliveries['batsman_runs'].sum() / len(venue_deliveries) * 100):.2f}%"""
        ...
        with col3:
            ...
            bowling_text = f"""**Bowling Metrics**
...
# ==================== COMPARE VENUES ====================
    ...
    if len(selected_venues) < 2:
        ...
        for venue in selected_venues:
            ...
            avg_wickets = venue_deliveries['is_wicket'].sum() / total_matches if total_matches > 0 else 0
            sr = (venue_deliveries['batsman_runs'].sum() / len(venue_deliveries) * 100) if len(venue_deliveries) > 0 else 0
            
            comparison_data.append({'Venue': venue, 'Matches': total_matches, 'Avg Runs/Match': avg_runs,
                                   'Avg Wickets': avg_wickets, 'Strike Rate': sr})
        ...
        with col1:
            fig_matches = px.bar(comp_df, x='Venue', y='Matches', title="Total Matches per Venue",
                               color='Matches', color_continuous_scale='Blues')
            st.plotly_chart(fig_matches, use_container_width=True)
        
        with col2:
            fig_runs = px.bar(comp_df, x='Venue', y='Avg Runs/Match', title="Average Runs per Match",
                            color='Avg Runs/Match', color_continuous_scale='RdYlGn')
            st.plotly_chart(fig_runs, use_container_width=True)
        
        col1, col2 = st.columns(2)
        with col1:
            fig_sr = px.bar(comp_df, x='Venue', y='Strike Rate', title="Overall Strike Rate",
                           color='Strike Rate', color_continuous_scale='Oranges')
            st.plotly_chart(fig_sr, use_container_width=True)
        
        with col2:
            fig_wickets = px.bar(comp_df, x='Venue', y='Avg Wickets', title="Average Wickets per Match",
                               color='Avg Wickets', color_continuous_scale='Reds')
...
# ==================== ALL VENUES OVERVIEW ====================
    ...
    for venue in all_venues:
        ...
        all_venue_data.append({'Venue': venue, 'Matches': total_matches, 'Avg Runs': avg_runs,
                             'Avg Wickets': avg_wickets, 'Strike Rate': sr})
    ...
    with col1:
        ...
        fig_all_matches = px.bar(all_venues_df.head(15), x='Venue', y='Matches',
                                title="Matches Hosted by Venue", color='Matches',
                                color_continuous_scale='Blues')
    ...
    with col2:
        ...
        fig_high_scoring = px.bar(top_scoring, x='Venue', y='Avg Runs',
                                 title="Highest Avg Runs/Match", color='Avg Runs',
    ...
    with col1:
        ...
        fig_sr_all = px.bar(high_sr, x='Venue', y='Strike Rate',
                           title="Highest Strike Rate", color='Strike Rate',
    ...
    with col2:
        ...
        fig_wickets_all = px.bar(high_wickets, x='Venue', y='Avg Wickets',
                                title="Most Wickets per Match", color='Avg Wickets',
